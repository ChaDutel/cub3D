...
static int	check_colors_values_norm(char c, char *str, int nb_values, int *j)
{
	if (nb_values != 2 && c != ',')
	{
		free(str);
...
static int	check_colors_values(t_config *config, int i, int j)
{
	...
	while (nb_values < 3)
	{
		...
		while (config->elems[i][j] && (config->elems[i][j] >= '0' && config->elems[i][j] <= '9'))
		{
			...
			{
				free(str);
				return (error_msg("Error : Color values are too big\n"));
			}
			j++;
		}
		while (config->elems[i][j] && config->elems[i][j] == ' ')
			j++;
		if (check_colors_values_norm(config->elems[i][j], str, nb_values, &j) == -1)
			return (-1);
		if (add_tab_colors_values(config, str, nb_values, i - 4) == -1)
		{
			free(str);
...
static int	check_colors(t_config *config)
{
	...
	// if (!config->colors)
	// 	return (error_msg("Error : 1malloc failed\n"));
	// config->colors[0] = malloc(sizeof(int) * 3);
	// if (!config->colors[0])
	// 	return (error_msg("Error : 2malloc failed\n"));
	// config->colors[1] = malloc(sizeof(int) * 3);
	// 	return (error_msg("Error : 3malloc failed\n"));
